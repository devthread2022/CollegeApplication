package com.jvt.devthread.collegeapplication.Exam;
...
import com.jvt.devthread.collegeapplication.Adapters.ExamAdapter;
...
import com.jvt.devthread.collegeapplication.databinding.FragmentExamsBinding;
...
public class Exams extends Fragment {
    ...
    private List<ExamModel> examModelList = new ArrayList<>();
    String uid,section;
    String regId;
    ...
    private void loadData(String section) {
        binding.examRecycler.setHasFixedSize(true);
        binding.examRecycler.setLayoutManager(new LinearLayoutManager(getContext(),LinearLayoutManager.VERTICAL,false));
        databaseReference.child("AvailableExam").child(section).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    examModelList.clear();
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                        ExamModel examModel = dataSnapshot.getValue(ExamModel.class);
                        examModelList.add(examModel);
                    }
                    ExamAdapter examAdapter = new ExamAdapter(getContext(),examModelList);
                    binding.examRecycler.setAdapter(examAdapter);
                }
            }
